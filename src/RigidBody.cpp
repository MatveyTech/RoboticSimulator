...
//	/*if (name.compare("link_2_r") == 0)
//		std::cout << name << DEG(rotAngle) << std::endl;*/
//	glRotated(DEG(rotAngle), rotAxis[0], rotAxis[1], rotAxis[2]);
//	/*if (name.compare("link_2_r") == 0)
//	{
//		Quaternion qq = getRotationQuaternion(rotAngle + 0.1, rotAxis);
//		state.orientation = qq;
//	}*/
//
...
Matrix3x3 RigidBody::getWorldMOI(const QuaternionR& orientation) {
	...
	while (!feof(fp)){
		...
		switch (lineType) {
			...
			case RB_MESH_NAME: {
					...
					string folder = string(DATA_FOLDER) + "/";
...
//						throwError("Incorrect rigid body input file - 3 arguments are required to specify a body point feature\n", line);
			...
			default:
				...
				Logger::print("RigidBody.cpp,Load from file - Do nothing with line: %s\n", buffer);
		}
	}
